...
(defn move
  "Moves coordinate in direction given. Returns new state"
...
;; Initially I tried solving this a simple queue that just moved one step at a time
...
;; this + priority map finally cracked it.
;; References:
...
;; PART 2
;; we generalize some funs

(defn gnext-moves-weighted
  "next moves with priority
   1. Moving towards end. 
   2. less accumulated heat"
  [minsteps maxsteps cities [cpos cheat cdir ccons]]
  (let [end [(dec (count (first cities))) (dec (count cities))]]
    (->> directions
         (remove #(= (mapv + cdir %) [0 0]))
         ;(map (move-fn cities [cpos cheat cdir ccons]))
         (mapcat (partial all-straight-moves cities [cpos cheat cdir ccons] minsteps maxsteps))
         ;(remove nil?)
         (map #(vector % ((partial prio-fn end) %))))))

(defn gtravel
  [minsteps maxsteps cities]
  (let [H (dec (count cities)) W (dec (count (first cities)))]
    (loop [Q1 (priority-map [[0 0] 0 nil 0] 0)
           visited {}]
      (let [[[node vheat dir _ :as visiting] _] (peek Q1)]
        (if (= node [H W])
          ...
          (if (get visited [node dir])
            (recur (pop Q1) visited)
            (let [nns ((partial gnext-moves-weighted minsteps maxsteps) cities visiting)]
              (recur (into (pop Q1) nns)
                     (assoc visited [node dir] vheat)))))))))

(comment
  (gtravel 1 3 example-data) ;; 102
  (gtravel 1 3 (parse-input (slurp "resources/input/2023/input_17"))))  ;; 694

(defn solution-2 [s]
  (->> s parse-input (gtravel 4 10)))

(comment
  (solution-2 example-input) ;; 94
  (solution-2 (slurp "resources/input/2023/input_17"))) ;; 829
